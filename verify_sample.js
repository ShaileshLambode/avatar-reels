...
async function checkSampleProgress() {
  try {
    ...
    if (reel.status.toLowerCase() === "animating") {
      ...
      // Let's check the size of the audio file in storage temp to calculate the exact frames
      const tempAudioPath = path.resolve(__dirname, `storage/temp/${reel._id}/audio.wav`);
      if (fs.existsSync(tempAudioPath)) {
        const stats = fs.statSync(tempAudioPath);
        // WAV file calculations: 24kHz 16bit Mono is 48000 bytes/sec
        ...
        // Find if there is any active python process to estimate progress
        ...
        try {
          ...
          if (processesRaw.trim()) {
            ...
            // Find active python processes running inference
            const activeInference = procList.find(p => p.CPU > 20); // usually active runs have higher CPU seconds accumulated
            
            if (activeInference) {
              const cpuSeconds = activeInference.CPU;
              // Under 2-CPU core mode, double threading speeds it up, making it ~1.0 second per frame
              const estSecPerFrame = 1.0;
              const estFramesDone = Math.min(totalFrames, Math.round(cpuSeconds / estSecPerFrame / 2)); // divided by 2 because CPU time is sum across all threads/cores
          ...
          // PowerShell might fail or return nothing, ignore telemetry and just print waiting
